...
struct malloc_state {
  // smallbins 对应的位图，用于标记对应的链表是否为空
  binmap_t   smallmap;
  // treebins 对应的位图，用于标记对应的树是否为空
  binmap_t   treemap;
  // 标记 dv chunk 的大小
  size_t     dvsize;
  // 标记 top chunk 的大小
  size_t     topsize;
  // 记录可获取的最小内存地址，用于做越界检查
  char*      least_addr;
  // 从最近一次申请的小于 256 字节的 chunk 块中分裂出来的 chunk 块，用于对
  // 应应用程序中连续的小内存分配申请，更好地利用其局部性原理提高程序性能。
  mchunkptr  dv;
  // 位于当前活跃的（最近一次用来进行内存分配的）segment 段的最顶端，不被任何
  // 分箱管理（treebins 或者 smallbins），位于顶端可以自由伸缩，大小可变。
  mchunkptr  top;
  // 记录将内存归还给操作系统的一个临界值，当 free 的内存总大小超过该值值时，
  // 会通过系统调用将内存归还给操作系统。
  size_t     trim_check;
  ...
  // 小块内存的箱体，每个箱子对应的是 chunk 块链表，范围：[8, 256]，共有 32 个箱子；
  // smallbins 中每个箱子管理的是一个确定的内存块，如 8、16、32 ... 256。
  mchunkptr  smallbins[(NSMALLBINS+1)*2];

  // 大块内存的箱体，每个箱子对应的是树状结构，范围：(256, ∞]，共有 32 个箱子；
  // treebins 中每个箱子管理的是一个内存范围，如 [256, 384)、[384, 512) ... [12572912, ∞）；
  // 因为管理的是一个范围，因此需要使用树状结构对该范围进行管理。树的规则如下：
  // - 1. 左子树上所有结点的值均小于右子树
  // - 2. 左右子树也均为树状结构（服从第 1 点）
  // 
  // 以第一个箱子为例，它管理的是范围是 256 ~ 384，它的树状结构如下：
  //                                box0          box1          box2      ...
  ...
  //                               /            \
  //                              /              \
  //                   T1[256, 320)              T2[320, 384) 
  //                   /          \                  / \
  //                  /            \                /   \
  //                 /              \              /     \ 
  //       T3[256, 288)             T4[288, 320)  T5      T6
  //       /         \                /   \
  //      /           \              /     \
  //    T7[256, 272)  T8[272, 288)  T9    T10
  //     /\
  //  T11 T12
